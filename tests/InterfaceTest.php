...
class InterfaceTest extends WebTestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$tmpdir .= DIRECTORY_SEPARATOR . 'idealist_' . md5(time() . mt_rand()) . DIRECTORY_SEPARATOR;
        ...
        // IdealistTest repository fixture
        $git->createRepository(self::$tmpdir . 'IdealistTest');
        $repository = $git->getRepository(self::$tmpdir . 'IdealistTest');
        file_put_contents(self::$tmpdir . 'IdealistTest/README.md', "## IdealistTest\nIdealistTest is a *test* repository!");
        file_put_contents(self::$tmpdir . 'IdealistTest/test.php', "<?php\necho 'Hello World'; // This is a test");
    ...
    public function testInitialPage()
    {
        ...
        $this->assertCount(1, $crawler->filter('div.repository-header a:contains("IdealistTest")'));
        $this->assertEquals('/IdealistTest/', $crawler->filter('.repository-header a')->eq(6)->attr('href'));
        $this->assertEquals('/IdealistTest/master/rss/', $crawler->filter('.repository-header a')->eq(7)->attr('href'));
    ...
    public function testRepositoryPage()
    {
        ...
        $crawler = $client->request('GET', '/IdealistTest/');
        $this->assertTrue($client->getResponse()->isOk());
        ...
        $this->assertEquals("## IdealistTest\nIdealistTest is a *test* repository!", $crawler->filter('#md-content')->eq(0)->text());
        $this->assertEquals('/IdealistTest/blob/master/README.md', $crawler->filter('.tree tr td')->eq(0)->filter('a')->eq(0)->attr('href'));
        $this->assertEquals('/IdealistTest/blob/master/test.php', $crawler->filter('.tree tr td')->eq(3)->filter('a')->eq(0)->attr('href'));
    ...
    public function testBlobPage()
    {
        ...
        $crawler = $client->request('GET', '/IdealistTest/blob/master/test.php');

        $this->assertTrue($client->getResponse()->isOk());
        $this->assertCount(1, $crawler->filter('.breadcrumb .active:contains("test.php")'));
        $this->assertEquals('/IdealistTest/raw/master/test.php',
                $crawler->filter('.source-header .btn-group a')->eq(0)->attr('href'));
        $this->assertEquals('/IdealistTest/blame/master/test.php',
                $crawler->filter('.source-header .btn-group a')->eq(1)->attr('href'));
        $this->assertEquals('/IdealistTest/commits/master/test.php',
    ...
    public function testRawPage()
    {
        ...
        $crawler = $client->request('GET', '/IdealistTest/raw/master/test.php');
        $this->assertTrue($client->getResponse()->isOk());
    ...
    public function testBlamePage()
    {
        ...
        $crawler = $client->request('GET', '/IdealistTest/blame/master/test.php');
        $this->assertTrue($client->getResponse()->isOk());
        $this->assertCount(1, $crawler->filter('.source-header .meta:contains("test.php")'));
        $this->assertRegexp('/\/IdealistTest\/commit\/[a-zA-Z0-9%]+/',
    ...
    public function testHistoryPage()
    {
        ...
        $crawler = $client->request('GET', '/IdealistTest/commits/master/test.php');
        $this->assertTrue($client->getResponse()->isOk());
        $this->assertEquals('Initial commit', $crawler->filter('.table tbody tr td h4')->eq(0)->text());

        $crawler = $client->request('GET', '/IdealistTest/commits/master/README.md');
        $this->assertTrue($client->getResponse()->isOk());
        $this->assertEquals('Initial commit', $crawler->filter('.table tbody tr td h4')->eq(0)->text());
    ...
    public function testCommitsPage()
    {
        ...
        $crawler = $client->request('GET', '/IdealistTest/commits');
        $this->assertTrue($client->getResponse()->isOk());
        $this->assertEquals('Initial commit', $crawler->filter('.table tbody tr td h4')->eq(0)->text());
    ...
    public function testStatsPage()
    {
        ...
        $crawler = $client->request('GET', '/IdealistTest/stats');
        $this->assertTrue($client->getResponse()->isOk());
    ...
    /**
     * @covers Idealist\Controller\MainController::connect
     */
    public function testRssPage()
    {
        $client = $this->createClient();

        $client->request('GET', '/IdealistTest/master/rss/');
        $response = $client->getResponse();

        $this->assertTrue($response->isOk());
        $this->assertRegexp('/Latest commits in IdealistTest:master/', $client->getResponse()->getContent());
        $this->assertRegexp('/Initial commit/', $client->getResponse()->getContent());
    }

    /**
     * @covers Idealist\Controller\TreeController::connect
     */
